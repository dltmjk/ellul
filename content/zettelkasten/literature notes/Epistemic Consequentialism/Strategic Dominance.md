...
The common setup of most game theory problems is that there exists a set of players :$$\left\{ 1, 2, \ldots, n \right\}$$ 
Each player $i$ has a set of strategies or moves they can make in a certain game state. Each move has a potential playoff, so if there are $m$ moves a player can make, there is an equivalent amount of payoffs. Lets say we have two strategies $A  \&  B$. 
